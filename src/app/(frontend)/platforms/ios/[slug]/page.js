...
export default function SecuritySingle({ params }) {
  ...
  const [Iosinsearch, setIosinsearch] = useState('');
  const [Slideperview, setslideperview] = useState(null);

  useEffect(() => {
    document.title = "ios > App Portal"
    if (slug) {
      ...
      <div className='ap-search-wrap'>
        <form className='ap-search-form' method=''>
          <div className="ap-search-text">
            <input type='text' className='form-control' value={Iosinsearch} onChange={(e) => setIosinsearch(e.target.value)} placeholder="Search Apps ...." />
